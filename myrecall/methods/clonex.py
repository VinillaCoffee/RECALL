...
class ClonEx_SAC(SAC):
    ...
    def get_best_return_head(self, num_episodes, current_task_idx, num_heads) -> int:
        ...
        for head_idx in range(current_task_idx):
            one_hot = one_hot_encoder.copy()  # 创建副本以避免修改原始数组
            ...
            for j in range(num_episodes):
                ...
                while not (done or (episode_len == self.max_episode_len)):
                    ...
                    one_hot_size = len(obs) - MW_OBS_LEN
                    new_one_hot = np.zeros(one_hot_size)
                    
                    # 确保head_idx在合理范围内，避免索引错误
                    if head_idx < num_heads and head_idx < one_hot_size:
                        new_one_hot[head_idx] = 1.0
