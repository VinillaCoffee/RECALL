...
# 将ContextRNN改为函数，而不是子类
def create_context_rnn(
    hidden_dim: int = 50,
    input_dim: int = None,
    history_length: int = 1,
    action_dim: int = None,
    ...
    """创建RNN模型用于处理历史信息"""
    # 定义输入
    prev_actions = tf.keras.layers.Input(shape=(history_length, action_dim), name="prev_actions")
    prev_rewards = tf.keras.layers.Input(shape=(history_length, 1), name="prev_rewards")
    prev_obs = tf.keras.layers.Input(shape=(history_length, obsr_dim), name="prev_obs")
    
    # 连接输入
    concat_input = tf.keras.layers.Concatenate(axis=-1)([prev_actions, prev_rewards, prev_obs])
    
    # GRU层
    gru_out = tf.keras.layers.GRU(hidden_dim)(concat_input)
    
    # 创建模型
    model = tf.keras.Model(
        inputs=[prev_actions, prev_rewards, prev_obs],
        outputs=gru_out,
        name="context_rnn"
    )
...
class RNNActor(MlpActor):
    ...
    def __init__(
        ...
        # 保存RNN相关参数并计算必要的值
        history_length_value = history_length
        context_dim_value = context_dim
        obsr_dim_value = obsr_dim
        
        # 计算context_input_dim
        if context_input_dim is None:
            context_input_dim_value = action_space.shape[0] + 1 + obsr_dim_value
        else:
            context_input_dim_value = context_input_dim
            
        # 首先调用父类初始化
        super(RNNActor, self).__init__(
            input_dim=input_dim + context_dim_value,  # 增加RNN输出的上下文维度
        ...
        # 初始化完成后再设置RNN相关属性
        self.history_length = history_length_value
        self.context_dim = context_dim_value
        self.obsr_dim = obsr_dim_value
        
        # 创建ContextRNN处理历史信息
        self.context_rnn = create_context_rnn(
            hidden_dim=context_dim_value,
            input_dim=context_input_dim_value,
            history_length=history_length_value,
            action_dim=action_space.shape[0],
            obsr_dim=obsr_dim_value,
        )
    ...
    def call(self, x, pre_act_rew=None, deterministic=False):
        ...
        if pre_act_rew is not None:
            # 从RNN获取上下文表示
            prev_actions, prev_rewards, prev_obs = pre_act_rew
            context = self.context_rnn([prev_actions, prev_rewards, prev_obs])
            # 将原始观察和上下文连接
    ...
    def trainable_variables(self):
        """返回包含RNN变量的可训练变量列表"""
        return super().trainable_variables + self.context_rnn.trainable_variables
    
    @property
    def common_variables(self) -> List[tf.Variable]:
        """获取模型中共享的参数列表，包括RNN部分"""
        if self.num_heads > 1:
            return self.core.trainable_variables + self.context_rnn.trainable_variables
        elif self.num_heads == 1:
            return (
                self.core.trainable_variables
                + self.head_mu.trainable_variables
                + self.head_log_std.trainable_variables
                + self.context_rnn.trainable_variables
            )
...
class RNNCritic(MlpCritic):
    ...
    def __init__(
        ...
        context_dim_value = context_dim
        obsr_dim_value = obsr_dim
        action_space_value = action_space
        
        # 计算context_input_dim
        if context_input_dim is None and action_space is not None:
            context_input_dim_value = action_space.shape[0] + 1 + obsr_dim_value  # 动作维度 + 奖励维度 + 观察维度
        else:
            context_input_dim_value = context_input_dim
        ...
        # 创建ContextRNN处理历史信息
        action_dim = None
        if action_space_value is not None:
            action_dim = action_space_value.shape[0]
    ...
    def common_variables(self) -> List[tf.Variable]:
        ...
        if self.num_heads > 1:
            ...
            return self.core.trainable_variables + self.head.trainable_variables + self.context_rnn.trainable_variables 